...
if card:
    ...
    # Print the first 500 chars of HTML
    print(f'\nHTML snippet:\n{str(card)[:800]}')
else:
    print('No property card found')
    # Let's look at the main container
    main = soup.select_one('main') or soup.select_one('[role="main"]') or soup.select_one('body > div')
    if main:
        print(f'\nMain container classes: {main.get("class")}')
